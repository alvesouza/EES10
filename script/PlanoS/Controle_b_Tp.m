...
faseraiz = +phase(q+b)/pi*180+Phi;

fprintf("Calculando a fase do (q+a)\n");
fprintf("faseraiz = phase(q+b)+Phi =phase(%f +j*(%f)) + (%f) = %f + (%f) \n",real(q+b),imag(q+b),Phi,phase(q+b)/pi*180,Phi);

fprintf("faseraiz = %f\n",faseraiz);
fprintf("a = -real(q) + imag(q)/tand(fasepolo)=-(%f) + (%f)/tan(%f)\n",real(q),imag(q),faseraiz);

a = -real(q) + imag(q)/tand(faseraiz);
fprintf("a = %f\n",a);
fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
