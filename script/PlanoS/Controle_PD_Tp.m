function C = Controle_PD_Tp(G,Mp,Tp,T1,T2,T3)
%Mp é o overshoot
%Tp tempo de pico
...
fprintf("K*abs(q+a) = (abs(q-polos))/(abs(k)*abs(q-raizes)*");
fprintf("exp(-real(q)*(T2+T3)))\n\n");

fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
fprintf("Calculando o valor de K*abs(q+a)\n\n");
