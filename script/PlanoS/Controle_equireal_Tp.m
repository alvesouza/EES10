function C = Controle_equireal_Tp(G,Mp,Tp,T1,T2,T3)
%Mp é o overshoot
%Tp tempo de pico
...
fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
fprintf("Calculando o valor de K*abs(q+aeq)*abs(q+ar)/(abs(q+beq)*abs(q+br))\n\n");
...
fprintf("\nK*abs(q+aeq)*abs(q+ar)/(abs(q+beq)*abs(q+br)) = %f\n\n\n",K);

fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
fprintf("dividindo os dois controladores com fase em que de Phi/2\n");
fprintf("novo Phi = %f/2 = %f\n",Phi,Phi/2)
Phi = Phi/2;
%Calculando o polo e a raiz do controlador
fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
fprintf("Para o controlador de raiz e polo equidistante\n")
fprintf("aeq = -real(q) + imag(q)/tand(90+Phi/2)=-(%f) + (%f)/tan(%f)\n",real(q),imag(q),90+Phi/2);
fprintf("beq = -real(q) + imag(q)/tand(90-Phi/2)=-(%f) - (%f)/tan(%f)\n",real(q),imag(q),90-Phi/2);
aeq = -real(q) + imag(q)/tand(90+Phi/2);
beq = -real(q) + imag(q)/tand(90-Phi/2);
fprintf("aeq = -real(q) + imag(q)/tand(Phi/2)=-(%f) + (%f)/tan(%f)\n",real(q),imag(q),Phi/2);
fprintf("aeq = %f\n",aeq);
fprintf("beq = %f\n",beq);
fprintf("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n\n");
fprintf("Para o controlador de raiz igual ao valor da parte real do polo desejado\n")

fprintf("ar = -real(q) =%f\n",-real(q));
fprintf("br = -real(q) + imag(q)/tand(90 - Phi)=-(%f) + (%f)/tan(90 - %f)\n",real(q),imag(q),Phi);
ar = -real(q);
br = -real(q) + imag(q)/tand(90 - Phi);
fprintf("br = %f\n",br);
fprintf("K = %f*abs(%f + i*(%f))/abs(%f + i*(%f))\n\n",K,real(q+br),imag(q+br),real(q+ar),imag(q+ar));
K = K*abs(q+br)/abs(q+ar);
