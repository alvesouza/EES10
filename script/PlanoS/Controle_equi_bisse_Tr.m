function C = Controle_equi_bisse_Tr(G,Mp,Tr,T1,T2,T3)
%Mp é o overshoot
...
raizes = cell2mat(z);
polos = cell2mat(p);
%T1,T2,T3 ~sao os tempos de retardo,1 é antes,2 na malha direta,3 malha de
%retorno
fprintf("Será encontrado o controlador em que o zero e polo possuem angulos equidistantes em relação a bissetriz\n");
...
beta = 180 - phase(q)*180/pi;
fprintf("beta = 180 - phase(q);\n");
fprintf("beta = %f\n",beta);
fprintf("a = -real(q) + imag(q)*tand((beta-Phi)/2)=%f + (%f)*tan(%f)\n",-real(q),imag(q),(beta-Phi)/2);
fprintf("b = -real(q) + imag(q)*tand((beta+Phi)/2)=%f + (%f)*tan(%f)\n",-real(q),imag(q),(beta+Phi)/2);
a = -real(q) + imag(q)*tand((beta-Phi)/2);
b = -real(q) + imag(q)*tand((beta+Phi)/2);
fprintf("a = %f\n",a);
fprintf("b = %f\n",b);
